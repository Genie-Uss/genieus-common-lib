...
public class EventEnvelopeParser {
  ...
  public Class<? extends DomainEvent> resolveEventType(String json) {
    try {
      ...
      if (node == null || node.isNull()) {
        throw new IllegalArgumentException("eventType 필드가 없습니다: " + json);
      }
      String eventType = node.asText();
      return EventTypeRegistry.resolve(eventType);
    } catch (Exception e) {
      throw new RuntimeException("eventType 추출 실패", e);
    }
  }
   ...
   */
  public EventEnvelope<? extends DomainEvent> deserialize(
